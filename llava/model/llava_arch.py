...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ##############
        #print("inside llava", images.shape)
        # if images.ndim == 4:
        #     images = images.unsqueeze(2)
            #########
        # vision_feature_dim = 768
        # projector_in_dim = 1024   # what your mm_projector currently expects
        
        # # Add adapter
        # self.vision_adapter = nn.Linear(vision_feature_dim, projector_in_dim)
        # image_features = self.get_model().get_vision_tower()(images)
        # #image_features = self.vision_adapter(image_features) 
        # #image_features = self.vision_adapter(image_features.to(self.get_model().mm_projector.device))
        # self.vision_adapter = self.vision_adapter.to(self.device)
        # image_features = self.vision_adapter(image_features.to(self.device))
        #     # --------------------------
        # # Step 3: Convert to sequence for LLaVA
        # # --------------------------
        # num_img_tokens = 4  # must match <image> tokens in prompt
        # image_features = image_features.unsqueeze(1).repeat(1, num_img_tokens, 1)  # [B, num_img_tokens, projector_in_dim]
        # image_features = image_features * 100.0  # scale factor, tune between 50-200
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            #print("cur_new_input_embeds ",len(cur_new_input_embeds))
