...
from data_utils.utils import load_annotation, load_image, binary_loader, load_image_rgb, enhance_image
...
class PromptSegmentDataset(Dataset):
    def __init__(
        ...
        self.mask_transform = transforms.Compose([
            transforms.Resize((256, 256)),      #### 1024x1024 --> 448x448
            transforms.ToTensor(),
        ])
    ...
    def process_image(self, image_path):                              ## this is for the LLAva image processing 
        ...
        image_pil = load_image_rgb(image_path, channel_indices=[2, 1, 0])      ### R, G, B band    channel_indices=[2, 1, 0]
        ...
        # image_tensor = self.image_processor(image_pil).unsqueeze(0)
        # #print("image_tensor ", image_tensor.shape)
        return image_tensor.squeeze(0).to(torch.float16) #image_tensor.to(torch.float16)
    
    def process_sam_image(self, image_path):          ### this is to be compatible with prithvi 
        image_pil = load_image(image_path, channel_indices= [0, 1, 2, 3, 4, 5])      #### Selected Band   channel_indices=[0, 1, 2, 3, 6, 7]
        ...
        image_tensor = F.interpolate(image_tensor.unsqueeze(0), size=(1024, 1024), mode='bilinear', align_corners=False)   #### 1024x1024 --> 448x448
        image_tensor = image_tensor.squeeze(0)  # (C, 1024, 1024)
        
        return image_tensor #image_sam_tensor.to(torch.float32)
    ...
    def __getitem__(self, idx):
        ...
        image_path = mask_path.replace("/masks", "/images").replace(".png", ".tif")      #### we did change png to tif
        ...
        image_tensor = self.process_image(image_path)    ##### i did the change s
